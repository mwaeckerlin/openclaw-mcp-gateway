...
// Helper: build a two-WS factory that serves ws1 on first call, ws2 on second.
function makeDualWsFactory(ws1: FakeWebSocket, ws2: FakeWebSocket): () => FakeWebSocket {
  let count = 0;
  return () => (count++ === 0 ? ws1 : ws2);
}

test("callGatewayRpc retries once after NOT_PAIRED — pairing endpoint called then succeeds", async () => {
  const ws1 = new FakeWebSocket();
  const ws2 = new FakeWebSocket();
  rpcTesting.setWebSocketFactory(makeDualWsFactory(ws1, ws2));
  ...
  globalThis.fetch = async (input: Parameters<typeof fetch>[0]) => {
    if (String(input).includes("/api/v1/pair")) {
      pairCallCount++;
      return makeJsonResponse(200, { ok: true, paired: true });
    }
    return makeResponse(500, "unexpected");
  };

  try {
    const promise = runAllowedToolWithArguments("openclaw_cron_list", {}, TEST_CONFIG, new Set(), deviceIdentity);

    // ws1: challenge → connect → Gateway returns NOT_PAIRED
    ws1.emitOpen();
    ws1.emitMessage({ type: "event", event: "connect.challenge", payload: { nonce: "nonce1" } });
    const frame1 = parseFrame(ws1.sent[0]!);
    ws1.emitMessage({
      type: "res",
      id: frame1.id,
      ok: false,
      error: { code: "NOT_PAIRED", message: "NOT_PAIRED: pairing required" }
    });

    // Allow the pairing HTTP call and the retry WS creation to complete.
    await new Promise<void>((r) => setImmediate(r));
    await new Promise<void>((r) => setImmediate(r));

    // ws2: challenge → connect OK → method result
    ws2.emitOpen();
    ws2.emitMessage({ type: "event", event: "connect.challenge", payload: { nonce: "nonce2" } });
    const frame2 = parseFrame(ws2.sent[0]!);
    ws2.emitMessage({ type: "res", id: frame2.id, ok: true, payload: {} });
    const methodFrame = parseFrame(ws2.sent[1]!);
    ws2.emitMessage({ type: "res", id: methodFrame.id, ok: true, payload: { jobs: [], total: 0 } });

    const result = await promise;
    assert.equal(pairCallCount, 1, "pairing endpoint must be called exactly once");
    assert.ok(result.includes("jobs"), "result must include jobs field");
  } finally {
...
test("callGatewayRpc propagates error when NOT_PAIRED after retry too", async () => {
  const ws1 = new FakeWebSocket();
  const ws2 = new FakeWebSocket();
  rpcTesting.setWebSocketFactory(makeDualWsFactory(ws1, ws2));
  ...
  try {
    ...
    // ws1: NOT_PAIRED on first attempt
    ws1.emitOpen();
    ws1.emitMessage({ type: "event", event: "connect.challenge", payload: { nonce: "n1" } });
    const f1 = parseFrame(ws1.sent[0]!);
    ws1.emitMessage({
      type: "res",
      id: f1.id,
      ok: false,
    ...
    // ws2: NOT_PAIRED again on retry — pairing endpoint returned 200 but Gateway still rejects
    ws2.emitOpen();
    ws2.emitMessage({ type: "event", event: "connect.challenge", payload: { nonce: "n2" } });
    const f2 = parseFrame(ws2.sent[0]!);
    ws2.emitMessage({
      type: "res",
      id: f2.id,
      ok: false,
    ...
    await assert.rejects(
      ...
      (error: unknown) =>
        error instanceof McpError &&
        error.code === ErrorCode.InternalError &&
        /pairing/i.test(error.message),
      "must surface pairing error after exhausting retry"
    );
  } finally {
    globalThis.fetch = savedFetch;
    rpcTesting.resetWebSocketFactory();
...
test("callGatewayRpc without deviceIdentity surfaces NOT_PAIRED as protocol error (no retry)", async () => {
  const ws = new FakeWebSocket();
  rpcTesting.setWebSocketFactory(() => ws);

  // No deviceIdentity provided → no pairing retry possible
  const promise = runAllowedToolWithArguments("openclaw_cron_list", {}, TEST_CONFIG);

  ws.emitOpen();
  ws.emitMessage({ type: "event", event: "connect.challenge", payload: { nonce: "n" } });
  const f = parseFrame(ws.sent[0]!);
  ws.emitMessage({
    type: "res",
    id: f.id,
    ok: false,
  ...
  await assert.rejects(
    ...
    "must propagate error when no identity is available for pairing retry"
  );
  rpcTesting.resetWebSocketFactory();
});
